...
export namespace XYZExample {
    ...
    const omvDataSourceHereRestApi_1 = new OmvDataSource({
        baseUrl: "https://mvtpoc.rendering.in.here.com/v3/all/mc",
        apiFormat: APIFormat.HereV1
    });
    mapView.addDataSource(omvDataSourceHereRestApi_1);
    ...
    function initializeMapView(id: string): MapView {
        ...
        sampleMapView.camera.position.set(0, 0, 32000000);
    ...
    function addGeoJsonDataSource(baseUrl: string, spaceId: string, token: string) {
        ...
        const geoJsonDataSource = new GeoJsonDataSource({ dataProvider: xyzDataProvider });
    ...
    function initTextForm() {
        ...
        if (themeField !== undefined) {
            ...
            (themeField as HTMLTextAreaElement).value = `
            {
                "styles": [
                    {
    ...
    function updateMapView(xyzUrl: string) {
        ...
        const dataSource = mapView.getDataSource("geojson");
    ...
    function updateGeoJsonTheme(theme: string) {
        const geoJsonDataSource = mapView.getDataSource("geojson");
        if (geoJsonDataSource !== undefined) {
            const geoJsonTheme = JSON.parse(theme);
            geoJsonDataSource.setTheme(geoJsonTheme);
        }
    }
}

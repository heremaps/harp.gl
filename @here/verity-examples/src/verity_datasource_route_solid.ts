...
const logger = LoggerManager.instance.create("verity_datasource_route_solid");
 ...
 *
 */
export namespace RouteDataSourceExampleSolidLines {
    /**
...
`;
    ...
    export class RoutingDataSource extends DataSource {
        ...
        constructor(name = "route") {
            ...
            this.line = new THREE.Mesh(new THREE.Geometry(), material);
        ...
        createGeometry(tile: Tile) {
            if (this.route && this.route.shape !== undefined) {
                ...
                for (let i = 0; i < this.route.shape.length; i += 3) {
                    ...
                    // (<THREE.Geometry>this.m_line.geometry).vertices.push(vertex);
                    line.push(vertex.x, vertex.y);
                }

                lines.add(line);

                this.line.geometry = lines.createGeometry();
            }

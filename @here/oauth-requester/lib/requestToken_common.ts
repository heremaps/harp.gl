...
export type Signer = (data: ArrayBufferLike, secretKey: string) => Promise<string>;
...
function signLatin1(data: string, secretKey: string, tokenSigner: Signer): Promise<string> {
    ...
    return tokenSigner(buf.buffer, secretKey);
}

function generateUid(): string {
    // ### TODO - find a better algorithm
    return (Math.random() * Number.MAX_SAFE_INTEGER).toFixed(16);
}
...
export async function requestToken_common(args: OAuthArgs,
    tokenSigner: Signer): Promise<Token> {

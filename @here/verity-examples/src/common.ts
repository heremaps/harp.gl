...
async function initalizeBearerTokenProvider(): Promise<AuthenticationCodeProvider | undefined> {
    ...
    try {
        ...
        return auth.getToken.bind(auth);
    } catch (err) {
        logger.info(
...
export async function bearerTokenProvider() {
    if (bearerTokenProviderPromise === undefined) {
        bearerTokenProviderPromise = initalizeBearerTokenProvider();
    }

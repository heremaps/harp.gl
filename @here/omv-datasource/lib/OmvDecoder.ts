...
import { Env, MapEnv, ValueMap } from "@here/datasource-protocol/lib/Theme";
import { GeoBox, Projection, TileKey, webMercatorTilingScheme } from "@here/geoutils";
import { LoggerManager, PerformanceTimer } from "@here/utils";
import * as Long from "long";
import * as THREE from "three";

import { ThemedTileDecoder, TileDecoderService } from "@here/mapview-decoder/index-worker";
...
export class OmvDecoder implements OmvVisitor {
    ...
    constructor(
        ...
        private readonly m_evaluator: ThemeEvaluator,
...
export class OmvTileDecoder extends ThemedTileDecoder {
    ...
    configure(theme: Theme, languages?: string[], options?: OptionsMap): void {
        super.configure(theme, languages, options);
 ...
 *
 * @deprecated Please use [[TileDecoderService.start]] with a proper decoder instance.
 */
export default class OmvWorkerClient {
    constructor(name?: string) {
        TileDecoderService.start(name ? name : OMV_TILE_DECODER_SERVICE_ID, new OmvTileDecoder());
    }
}

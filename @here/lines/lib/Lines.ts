...
export function createLineGeometry(
    ...
    const isClosed = polyline[0] === polyline[polyline.length - 2]
                  && polyline[1] === polyline[polyline.length - 1];
    for (let i = 0; i < pointCount; ++i) {
        const x = polyline[i * 2];
        const y = polyline[i * 2 + 1];
        const T1 = (isClosed && i === 0)
            ? tangents[tangents.length - 1] : tangents[Math.max(0, i - 1)];
        const T2 = (isClosed && i === pointCount - 1)
            ? tangents[0] : tangents[Math.min(i, tangents.length - 1)];
        if (i > 0) {
            const L1 = L[i - 1];
            const L2 = L[i];
            if (!highPrecision) {

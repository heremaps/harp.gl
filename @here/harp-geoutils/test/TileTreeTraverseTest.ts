...
class TestSubdivisionScheme implements SubdivisionScheme {
    getSubdivisionX(level: number): number { return 0 };
    getSubdivisionY(level: number): number { return 0 };
    getLevelDimensionX(level: number): number { return 0 };
    getLevelDimensionY(level: number): number { return 0 };
}
...
describe("TileTreeTraverse", function() {
    ...
    it("get tiles of 1x1 subdivison", function() {
        ...
        const result1 = ttt.subTiles(TileKey.fromRowColumnLevel(0, 0, 0));
        assert.equal(result1.length, 1);
        assert.include(result1[0], { row: 0, column: 0, level: 1 });

        const result2 = ttt.subTiles(result1[0]);
        assert.equal(result2.length, 1);
        assert.include(result2[0], { row: 0, column: 0, level: 2 });

        const result3 = ttt.subTiles(result2[0]);
        assert.equal(result3.length, 1);
        assert.include(result3[0], { row: 0, column: 0, level: 3 });

        const result4 = ttt.subTiles(TileKey.fromRowColumnLevel(0, 0, 19));
        assert.equal(result4.length, 1);
        assert.include(result4[0], { row: 0, column: 0, level: 20 });
    });
    ...
    it("get tiles of 2x1 subdivison", function() {
        ...
        const result2 = ttt.subTiles(TileKey.fromRowColumnLevel(0, 1, 1));
        assert.equal(result2.length, 2);
        assert.deepInclude(result2, { row: 0, column: 2, level: 2 });
        assert.deepInclude(result2, { row: 0, column: 3, level: 2 });

        const result3 = ttt.subTiles(TileKey.fromRowColumnLevel(0, 2, 2));
        ...
        const result4 = ttt.subTiles(TileKey.fromRowColumnLevel(0, 9, 14));
    ...
    it("get tiles of 2x2 subdivison", function() {
        ...
        const result2 = ttt.subTiles(TileKey.fromRowColumnLevel(11, 7, 4));
    ...
    it("get tiles of double umbrella subdivison", function() {
        ...
        const result2 = ttt.subTiles(TileKey.fromRowColumnLevel(1, 61, 7));
        assert.equal(result2.length, 2);
        assert.deepInclude(result2, { row: 1, column: 122, level: 8 });
        assert.deepInclude(result2, { row: 1, column: 123, level: 8 });

        const result3 = ttt.subTiles(TileKey.fromRowColumnLevel(0, 44, 15));
        assert.equal(result3.length, 2);

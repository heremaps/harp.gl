...
export class GeoJsonTileDecoder extends ThemedTileDecoder {
    ...
    decodeThemedTile(
        ...
        const decoder = new GeoJsonDecoder(projection, themeEvaluator);
...
class GeoJsonDecoder {
     ...
     * @param m_evaluator theme evaluator
     */
    constructor(private readonly m_projection: Projection, private m_evaluator: ThemeEvaluator) {}
    ...
    getDecodedTile(tileKey: TileKey, data: GeoJsonDataType): DecodedTile {
        ...
        return {
            ...
            techniques: this.m_evaluator.techniques
        };
    }
    ...
    private findOrCreateTechniqueIndex(feature: Feature, envType: Value): number {
        ...
        if (feature.properties && feature.properties.featureClass !== undefined) {
            featureDetails.featureClass = feature.properties.featureClass;
        }

        const env = new MapEnv({ type: envType, ...featureDetails });
        const techniques = this.m_evaluator.getMatchingTechniques(env);
        let techniqueIndex;
        ...
        if (
            ...
            // if needed technique not found, create new one and update theme
            if (techniqueIndex === undefined) {
                let newStyle = this.m_evaluator.theme.styles.find(
                ...
                if (newStyle !== undefined) {
                    ...
                    this.m_evaluator.theme.styles.push(newStyle);

                    // set _index for new style
                    this.m_evaluator.getMatchingTechniques(env);

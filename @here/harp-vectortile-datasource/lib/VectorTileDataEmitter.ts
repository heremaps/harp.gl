    ...
    processPolygonFeature(
        ...
        techniques.forEach(technique => {
            ...
            for (const polygon of geometry) {
                const rings: Ring[] = [];
                let discardPolygon: boolean = false;
                for (let i = 0; i < polygon.rings.length; ) {
                    ...
                    if (shouldClipPolygons) {
                        ...
                        if (hasCoordsOutsideTileBounds) {
                            const winding = Math.sign(THREE.ShapeUtils.area(ringCoords));
                            ...
                            // │                  │
                            // └─────────<────────┘
                            // So, if one exterior ring get clipped to a null-area polygon we
                            ...
                            if (!THREE.ShapeUtils.area(ringCoords)) {
                                if (winding > 0) {
                                    discardPolygon = true;
                                    break;
                                } else {
                                    // Skip the current hole only
                                    continue;
                                }
                            }
                        }
                    ...
                    const ring = new Ring(ringCoords, textureCoords, extents, shouldClipPolygons);
                    rings.push(ring);
                }
                if (!discardPolygon && rings.length) {
                    polygons.push(rings);
                }
            }

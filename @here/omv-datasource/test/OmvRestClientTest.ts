...
import { DownloadRequestInit, DownloadResponse, Headers } from "@here/fetch";
import { TileKey } from "@here/geoutils";
import { assert } from "chai";
import * as sinon from "sinon";
import { APIFormat, OmvRestClient } from "../index";
...
class MockDownloadManager extends DownloadManager {
    download(tileUrl: string): Promise<DownloadResponse> {
        return Promise.resolve(createMockDownloadResponse(tileUrl));
    }
}
...
describe("OmvRestClient", () => {
    ...
    it("generates authentication header with bearer token from a function", async () => {
        const restClient = new OmvRestClient({
            ...
            apiFormat: APIFormat.HereV1,
            downloadManager: mockDownloadManager,
            getBearerToken: async () => Promise.resolve("12345")
        });
        await restClient.getTile(new TileKey(1, 2, 3));
        assert.equal(downloadSpy.args[0][0], "https://some.base.url/3/2/1/omv");
        const downloadRequestInit = downloadSpy.args[0][1] as DownloadRequestInit;
        assert.exists(downloadRequestInit);
        assert.exists(downloadRequestInit.headers);
        if (downloadRequestInit.headers !== undefined) {

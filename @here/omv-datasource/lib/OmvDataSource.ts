...
import { GeometryType, ITileDecoder, Theme } from "@here/datasource-protocol";
...
import {
    DataProvider,
    HypeDataProvider,
    HypeDataProviderOptions,
    TileDataSource,
    TileFactory
} from "@here/mapview-decoder";
import {
    FeatureModifierId,
...
export interface OmvDataSourceParameters {
    /**
     * The name of the [[DataSource]]. Default is `"omv"`.
     */
    name?: string;
...
function getDataProvider(
    params: OmvWithRestClientParams | OmvWithDataStoreClientParams | OmvWithCustomDataProvider
) {
    if ((params as OmvWithCustomDataProvider).dataProvider) {
        ...
        return new HypeDataProvider({
            ...(params as HypeDataProviderOptions & DataStoreClientParameters),
            layer: params.layerName || "mvt"
        });
    }
}

export type OmvWithRestClientParams = OmvRestClientParameters & OmvDataSourceParameters;
export type OmvWithDataStoreClientParams = OmvDataSourceParameters & DataStoreClientParameters;
...
export class OmvDataSource extends TileDataSource<OmvTile> {
    ...
    setTheme(theme?: Theme, languages?: string[]): void {
        ...
        this.decoder.configure(theme, languages, this.m_decoderOptions);

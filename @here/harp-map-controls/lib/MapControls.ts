...
export class MapControls extends THREE.EventDispatcher {
    ...
    /**
     * Changes the pitch to the desired value. Supports inertia.
     *
     * @param angle Desired pitch angle in radian.
     */
    rotatePitchRadian(angle: number): void {
        if (this.inertiaEnabled && this.m_zoomIsAnimated) {
            this.stopZoom();
        }

        const initialPitch = this.currentPitch;
        const deltaAngle = angle - initialPitch;
        pitchQuaternion.setFromAxisAngle(pitchAxis, deltaAngle);

        yawQuaternion.multiply(pitchQuaternion);

        const oldCameraTarget = MapViewUtils.rayCastWorldCoordinates(this.mapView, 0, 0);
        const oldCameraDistance = this.mapView.camera.position.z / Math.cos(initialPitch);

        this.mapView.camera.quaternion.copy(yawQuaternion);
        this.mapView.camera.updateMatrixWorld(false);
        this.mapView.camera.position.z = Math.cos(this.currentPitch) * oldCameraDistance;

        const newCameraTarget = MapViewUtils.rayCastWorldCoordinates(this.mapView, 0, 0);
        if (newCameraTarget === null || oldCameraTarget === null) {
            this.updateMapView();
            return;
        }

        const panVector = oldCameraTarget.sub(newCameraTarget);
        MapViewUtils.pan(this.mapView, panVector.x, panVector.y);
    }
     ...
     */
    toggleTilt(): void {
        const aimTilt = this.currentPitch < EPSILON_PITCH || this.m_tiltState === TiltState.Down;
    ...
    private get currentPitch(): number {
        return MapViewUtils.extractYawPitchRoll(this.camera.quaternion).pitch;
    }

    private get targettedPitch(): number {
        return this.m_targettedPitch === undefined ? this.currentPitch : this.m_targettedPitch;
    }
    ...
    private tilt() {
        ...
        this.m_currentPitch = this.inertiaEnabled
            ...
            : this.targettedPitch;
        this.rotatePitchRadian(this.m_currentPitch);

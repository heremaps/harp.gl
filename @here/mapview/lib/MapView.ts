...
import { createLight, ImageTexture, Light, Sky, Theme } from "@here/datasource-protocol";
import { GeoCoordinates, MathUtils, mercatorProjection, Projection } from "@here/geoutils";
import { TextMaterial } from "@here/materials";
import { TextMaterialConstructor } from "@here/text-renderer";
...
export class MapView extends THREE.EventDispatcher {
     ...
     */
    getDataSource(dataSourceName: string): DataSource | undefined {
        for (const dataSource of this.m_tileDataSources) {
    ...
    addDataSource(dataSource: DataSource): Promise<void> {
        ...
        return dataSource
            ...
            .then(() => {
                ...
                dataSource.setTheme(this.m_theme, this.m_languages);
    ...
    getScreenPosition(geoPos: GeoCoordinates): THREE.Vector2 | undefined {
        ...
        if (p !== undefined) {
            ...
            p.x = (p.x + width / 2) / window.window.devicePixelRatio;
            p.y = (height - (p.y + height / 2)) / window.window.devicePixelRatio;
        }
        return p;
    }
    ...
    resize(width: number, height: number) {
        ...
        this.m_renderer.setPixelRatio(window.window.devicePixelRatio);
    ...
    private initTheme() {
        ...
        const themePromise =
            typeof this.m_options.theme === "string"
                ...
                : Promise.resolve<Theme | undefined>(this.m_options.theme);

        themePromise.then((theme: Theme | undefined) => {
            this.m_theme = theme;

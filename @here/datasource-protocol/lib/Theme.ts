...
function isIdentChar(codepoint: number): boolean {
    return (
        isLetterOrNumber(codepoint) || codepoint === Character._ || codepoint === Character.Dollar
    );
}
...
export interface Style {
    /**
     * Optional name of the [[DataSource]].
     */
    dataSource?: string;
    ...
    /**
     * Optimization: Index in the table of [[ThemeEvaluator]].
     */
    _index?: number;
...
export class ThemeEvaluator {
    ...
    getMatchingTechniques(env: MapEnv): Technique[] {
        ...
        for (const currStyle of this.theme.styles) {
    ...
    private processStyle(
        ...
        if (style.dataSource !== undefined && this.dataSourceName !== undefined) {
            if (style.dataSource !== this.dataSourceName) {
                return false;
            }
        }
...
export class ThemeVisitor {
    ...
    visitStyles(visitFunc: (style: Style) => boolean): boolean {
        ...
        const styles = this.theme.styles;

        for (const currStyle of styles) {
            if (visit(currStyle)) {
                return true;
            }
        }

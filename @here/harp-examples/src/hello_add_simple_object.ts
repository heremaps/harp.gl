 ...
 * ```typescript
 * [[include:harp_gl_hello_add_simple_object_0.ts]]
 * ```
 *
 * Here the object is added to the scene and we force the map to be re-rendered, to ensure the
 * object is now visible.
 * ```typescript
 * [[include:harp_gl_hello_add_simple_object_1.ts]]
 * ```
 *
 * Next we need to position the cube, we use the helpful method [[getWorldPositionAt]] from the
 * [[MapView]] to get the world space position under the mouse when it is clicked, this is shown
 * here:
 * ```typescript
 * [[include:harp_gl_hello_add_simple_object_2.ts]]
 * ```
 *
 ...
 * ```typescript
 * [[include:harp_gl_hello_add_simple_object_3.ts]]
 * ```
 */
export namespace HelloWorldAddSimpleObject {
    // snippet:harp_gl_hello_add_simple_object_0.ts
    const scale = 100;
    ...
    // end:harp_gl_hello_add_simple_object_0.ts
    ...
    function initializeMapView(id: string): MapView {
        ...
        canvas.addEventListener("mousedown", event => {
            // snippet:harp_gl_hello_add_simple_object_1.ts
            const cube = createPinkCube();
            map.scene.add(cube);
            // Force the scene to be rerendered once the cube is added to the scene.
            map.update();
            // end:harp_gl_hello_add_simple_object_1.ts

            // snippet:harp_gl_hello_add_simple_object_2.ts
            // Get the position of the mouse in world space.
            const worldPositionAtMouse = map.getWorldPositionAt(event.pageX, event.pageY);
            if (worldPositionAtMouse === null) {
                return;
            }
            // end:harp_gl_hello_add_simple_object_2.ts

            // snippet:harp_gl_hello_add_simple_object_3.ts
            ...
            map.addEventListener(MapViewEventNames.Render, () => {
                ...
                cube.position.copy(worldPositionAtMouse);
                cube.position.sub(map.worldCenter);
            });
            // end:harp_gl_hello_add_simple_object_3.ts
        });
    ...
    message.innerHTML = `Click to add a 10m^3 pink box to the center of the current view.`;

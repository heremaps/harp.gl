...
export class OmvDecodedTileEmitter implements IOmvEmitter {
        ...
        for (const technique of techniques) {
            ...
            if (techniqueName === "line") {
                ...
                const text: Value =
                    typeof textLabel === "string"
                        ? env.lookup(textLabel)
                        : OmvDecoder.getFeatureName(env, useAbbreviation, useIsoCode,
                            this.m_languages);
    ...
    processPolygonFeature(
        ...
        const isExtruded =
            technique.name === "extruded-polygon" && env.lookup("extrude") === "true";
        ...
        if (isExtruded) {
            ...
            minHeight = (env.lookup("min_height") as number) * scaleFactor;
            height = (env.lookup("height") as number) * scaleFactor;
        }
        ...
        if (technique.name === "extruded-polygon") {
            const colorValue = env.lookup("color");
    ...
    private createGeometries(): any {
        this.m_meshBuffers.forEach((meshBuffers, techniqueIdx) => {
            ...
            if (
                !this.m_evaluator.techniques ||
                this.m_evaluator.techniques.length <= techniqueIdx
            ) {
                throw new Error("Invalid technique index");
            }

            const technique = this.m_evaluator.techniques[techniqueIdx];
            if (technique === undefined) {
                return;
            }
